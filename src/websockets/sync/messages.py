...
class Assembler:
    ...
    def get(self, timeout: Optional[float] = None) -> Optional[Data]:
        ...
        If ``timeout`` is set and elapses before a complete message is received,
        :meth:`get` returns ``None``.

        :raises EOFError: if the stream of frames has ended
        :raises RuntimeError: if two threads run :meth:`get` or
    ...
    def put(self, frame: Frame) -> None:
        ...
        fully consuming the return value of :meth:`get_iter`.

        :meth:`put` assumes that the stream of frames respects the protocol.
        If it doesn't, the behavior is undefined.

        :raises EOFError: if the stream of frames has ended
        :raises RuntimeError: if two threads call :meth:`put` concurrently
